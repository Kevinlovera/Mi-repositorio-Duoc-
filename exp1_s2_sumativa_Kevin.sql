/* BORRADO DE LAS TABLAS SI YA EST�N CREADAS */
...
DECLARE
    ...
    v_empleados_procesados NUMBER := 0;

    -- Variables usuario
    v_usuario           VARCHAR2(20);
    v_letra_estado      VARCHAR2(1);
    v_nombre3           VARCHAR2(3);
    v_largo_nombre      NUMBER;
    v_ultimo_dig_sueldo NUMBER;
    v_anios_trabajo     NUMBER;

    -- Variables clave
    v_clave_usuario    VARCHAR2(20);
    v_tercer_dig_run   VARCHAR2(3);
    v_anio_nac_mod     NUMBER;
    v_sueldo_mod       NUMBER;
    v_letras_apellido  VARCHAR2(2);
    v_mes_bd           NUMBER := EXTRACT(MONTH FROM SYSDATE);
    v_anio_bd          NUMBER := EXTRACT(YEAR FROM SYSDATE);
...
BEGIN
    ...
    FOR emp IN (
        ...
        -- USUARIO
        v_letra_estado := SUBSTR(LOWER(emp.nombre_estado_civil),1,1);
        v_nombre3 := LOWER(SUBSTR(emp.pnombre_emp,1,3));
        v_largo_nombre := LENGTH(emp.pnombre_emp);
        v_ultimo_dig_sueldo := MOD(emp.sueldo_base,10);
        v_anios_trabajo :=
            ROUND(MONTHS_BETWEEN(v_fecha_proceso, emp.fecha_contrato)/12);
        ...
        -- CLAVE
        v_tercer_dig_run := SUBSTR(emp.numrun_emp,1,3);
        v_anio_nac_mod := EXTRACT(YEAR FROM emp.fecha_nac) + 2;
        v_sueldo_mod := MOD(emp.sueldo_base,100) - 1;

        IF emp.nombre_estado_civil IN ('Casado','Acuerdo de union civil') THEN
            v_letras_apellido := LOWER(SUBSTR(emp.appaterno_emp,1,2));
        ELSIF emp.nombre_estado_civil IN ('Soltero','Divorciado') THEN
            v_letras_apellido :=
                LOWER(SUBSTR(emp.appaterno_emp,1,1)) ||
                LOWER(SUBSTR(emp.appaterno_emp,-1,1));
        ELSIF emp.nombre_estado_civil = 'Viudo' THEN
            v_letras_apellido :=
                LOWER(SUBSTR(emp.appaterno_emp,-2,2));
        ELSE
            v_letras_apellido :=
                LOWER(SUBSTR(emp.appaterno_emp,-2,2));
        END IF;
        ...
        VALUES (
            ...
            emp.numrun_emp,
            emp.dvrun_emp,
            emp.pnombre_emp || ' ' || emp.appaterno_emp,
            v_usuario,
            v_clave_usuario
        );

        v_empleados_procesados := v_empleados_procesados + 1;

    END LOOP;

    -- CONTROL
    IF v_empleados_procesados = v_total_empleados THEN
        COMMIT;
    ELSE
        ROLLBACK;
    END IF;
